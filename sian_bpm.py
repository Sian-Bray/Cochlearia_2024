...
His Fst's:  0.05 and 0.0.  Mine: 0.093 and 0.131"""
...
def calcBPM(input_file, output, outname, window_size, minimum_snps, num_pops):
    ...
    def NestedAnova(locus_list):
        locus = []
        for l in locus_list:  # Remove missing data from the site ##for one site and both pops
        ...
        for pop_site in locus: ##for one site and one pop
            FSNij2temp = 0.0
            ...
            ac = sum([float(geno) for geno in pop_site[6:]])  # Calculate ac for a population ##needed
            n_i.append(nnn) ##no individuals sequenced at the site
            try:
                p_i.append(ac / an)
        ...
        h_m1 = (ac_m[0]*(an_m[0]-ac_m[0]))/(an_m[0]*(an_m[0]-1)) ###
        h_m2 = (ac_m[1]*(an_m[1]-ac_m[1]))/(an_m[1]*(an_m[1]-1)) ###
        p_avr = (ac_m[0]/an_m[0]+ac_m[1]/an_m[1])/2
        Nhudson = (ac_m[0]/an_m[0]-ac_m[1]/an_m[1])**2 - h_m1/an_m[0] - h_m2/an_m[1]###
        Dhudson = Nhudson + h_m1 + h_m2###
        Nnei = (((ac_m[0]/an_m[0])-(ac_m[1]/an_m[1]))**2)/2 # to make maximum 1 instead of 2 (Bhatia, 2013)
        Dnei = 2 * p_avr * (1-p_avr) # to make maximum 1 instead of 2 (Bhatia, 2013)
        ...
        for i, pop in enumerate(ac_ij): ##i for zeroth or first pop
        ...
        return rnum, rden, fnum, fden, Nhudson, Dhudson, Nnei, Dnei, poly
    ...
    # Prepare output file
    outfile = output + outname + '_BPM.txt' # Create string for output file name
    out1 = open(outfile, 'w') # Create output file
    out1.write("outname\tscaff\tstart\tend\twin_size\tnum_sites\tnum_snps\tRho\tFstWC\tdxy\tAFD\tFixedDiff\tFstH\tFstN\n") # Add headder
    ...
    with open(input_file, 'r') as data:
        for i, line in enumerate(data):
            line = line.strip("\n").strip("\t").split("\t") # Split the line
            try:
                pop, ploidy, scaff, pos, an, dp = line[:6] # Assign items in line names
            except:
                print(line[:6])
            pos = float(pos)

            if i % 100000 == 0:
                print(i) # Print which line you are on every 100'000 lines
            if i == 0: # For the very first line only
                old_pos = pos
            ...
            if pos > start and pos <= end and scaff == oldscaff: # If you are within the same window
                if pos == old_pos:  # Accruing information from multiple populations but same locus
                    ...
                    rnum, rden, fnum, fden, Nhudson, Dhudson, Nnei, Dnei, poly = NestedAnova(Locus)
                    if poly is True:
                        if num_pops == 2:
                            d, a = calcDxy(Locus)
                            dxy += d
                        ...
                        snp_count += 1
                        site_count += 1
                        fst = [sum(x) for x in zip(fst, [fnum, fden])]  # Adds numerator and denominators from current 
                        rho = [sum(x) for x in zip(rho, [rnum, rden])]
                        fstH = [sum(x) for x in zip(fstH, [Nhudson, Dhudson])]
                        fstN = [sum(x) for x in zip(fstN, [Nnei, Dnei])]
                ...
                if snp_count >= minimum_snps:  # If there are enough SNPs. Report or exclude window.
                    Snp_count += snp_count
                    ...
                    try:
                        ...
                        out1.write(outname + '\t' + oldscaff + '\t' +  # Write calculations to file
                                   str(start) + '\t' +
                                   str(end) + '\t' +
                                   str(window_size) + '\t' +
                                   str(site_count) + '\t' +
                        ...
                        print('ZeroDivisionError:', snp_count, fst, rho, fstH, fstN, Fst, Rho, FstH, FstN)
    ...
    rho_G = FAC / (1 + FAC)
    Fst_G = Fst[0] / Fst[1]
    Fst_GH = FstH[0] / FstH[1]
    Fst_GN = FstN[0] / FstN[1]
    if num_pops == 2:
        Dxy = Dxy / float(Site_count)
        AFD = AFD / float(Site_count)
    else:
...
if __name__ == '__main__':  # The body of this if statement only runs if bpm.py is the main program, and not if it has been imported by another program.
    ...
    j1, j2, j3 = calcBPM(args.i, args.o, args.prefix, args.ws, args.ms, args.np) # This is the run command
